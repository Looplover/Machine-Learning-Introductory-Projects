...
EPOCHS = 375
PATIENCE = 10
MODEL_SAVE_PATH = "consonant_model.pkl" 
...
def train_architecture(arch, X_train, Y_hot_train, X_test, Y_test):
    ...
    nn = NeuralNetwork(layer_dims, activation='relu')
    ...
    if arch == [512, 256, 128, 64]:
        print(f"Saving 4-layer model (ReLU) to {MODEL_SAVE_PATH}...")
...
def main(train_path, test_path, output_folder):
    ...
    print("\n--- Part (d): Experimenting with Network Depth (ReLU) ---")
    ...
    plt.savefig(os.path.join(output_folder, "plot_d.png"))
    print(f"\nPlot saved to {os.path.join(output_folder, 'plot_d.png')}")
    
    final_predictions = np.concatenate(all_predictions)
    output_df = pd.DataFrame({'prediction': final_predictions})
    output_csv_path = os.path.join(output_folder, 'prediction_d.csv')
